/**
 ...
 \file
 Implementation of class ParticleID.
...
namespace Smear {
   ...
   int ParticleID::Wild(int pbin, int trueID) {
      ...
      for(unsigned i(0); i < values.size(); i++) {
         ...
         else if(r > values.at(i-1) and r < values.at(i)) {
            falseid = FalseIdent.at(i); 
         } // else if
      } // for
      return falseid;
   ...
   void ParticleID::Clear(Option_t*) {
      TrueIdent.clear();
      FalseIdent.clear();
      PMin.clear();
      PMax.clear();
   ...
   void ParticleID::ReadP(TString filename) {
      ...
      while(std::getline(Qfile, line).good()) {
         ...
         // Read number of momentum bins
            ...
            PMin.resize(pbin);
            PMax.resize(pbin);
            gotBins = not PMin.empty();
         } // if
         // Read the probabilities for this momentum bin/true ID/false ID
         else if(starts(line, "1") or starts(line, "2") or starts(line, "3")) {
            if(not (gotTrue and gotFalse and gotBins)) {
   ...
   void ParticleID::DevSmear(const erhic::ParticleMC& prt,
                             ParticleMCS& prtOut) {
      double momentum(0.);
      if(bUseMC) {
         momentum = prt.GetP();
      ...
      if(InListOfTrue(pid) not_eq -1 and Accept.Is(prt)) {
         for(unsigned i(0); i < PMin.size(); i++) {
            if(momentum > PMin.at(i) and momentum < PMax.at(i)) {
